...
import { areAllTicketsBooked, getEventStatus, getSimilarEvents, isNearbyWithUserLocation } from '../event-helper';
...
export default async function EventDetailsPage ( { params }:{params:Promise<{ eventId: string }>}) {
  ...
  const [events, setEvents] = useState<EventDataObjResponse[]>([])
  ...
  const fetchEvents = async () => {
    ...
    if(result && result.success && result.data.length > 0) {
        ...
        await setEvents(receivedArrayObj)
    } else {
  ...
  const similarEvents = getSimilarEvents(events,eventId)
  return (
